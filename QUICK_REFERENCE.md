...
- Removes labels not in the manifest (requires `prune: true`)
...
- Use `build-mode: autobuild` (recommended); `manual` is accepted as an input but will not produce correct results because the reusable workflow has no step between CodeQL init and analyze for injecting custom build commands
